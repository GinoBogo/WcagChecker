...
class TestWCAGCheckerApp(unittest.TestCase):
    ...
    def test_random_colors_validity(self):
        ...
        for state_key, _ in self.app.button_state_definitions:
            ...
            self.assertTrue(self._is_hex_color(bg_color), f"Invalid background color: {bg_color} for state {state_key}")
            self.assertTrue(self._is_hex_color(fg_color), f"Invalid foreground color: {fg_color} for state {state_key}")
    ...
    def test_random_colors_default_button_unchanged(self):
        ...
        self.assertEqual(initial_default_bg, final_default_bg,
    ...
    def test_random_colors_compliance(self):
        ...
        min_contrast_button_bg_vs_button_fg = 4.5 # Relaxed test requirement to WCAG AA normal text
    ...
    def test_random_colors_distinctness(self):
        ...
        self.assertEqual(initial_colors["default"]["background"], generated_colors["default"]["background"])

        # Other states should ideally change
        changed_count = 0
        for state_key in self.app.state_descriptions:
            if state_key != "default":
                if (initial_colors[state_key]["background"] != generated_colors[state_key]["background"] or
                    initial_colors[state_key]["foreground"] != generated_colors[state_key]["foreground"]):
                    changed_count += 1
        ...
        self.assertGreater(changed_count, len(self.app.button_state_definitions) - 2,
